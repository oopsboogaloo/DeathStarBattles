...
- [x] **11.7** Tournament: surviving teams claim collectables — add a tournament option. When enabled, at the end of a game all collectables remaining on the map are distributed as random weapons to the surviving teams (one collectable per team, round-robin if multiple collectables). Uses the standard 80/16/4% weapon tier weighting.
  - [x] **11.7a** Resolve the claim in-game at the end of the round (during the results display, before the game-over screen) instead of silently after the round. Each claimed collectable plays the same shatter + grant-name VFX as a normal pickup, in the collecting team's colour, and the results display is held long enough for the animation to finish so players can see who claimed what.
  - [x] **11.7b** Give the collectable grant label a quick fade-in / fast eased fade-out so it disappears gracefully instead of cutting off abruptly.
- [x] **11.8** Starting weapons for non-dev mode — the Starting Weapons config option is now visible to all players with new loadouts: None; Basic (top up to 1 Triple Cannon + 1 Force Shield); Marines (2 Blasters + 2 Blunderbusses); Demolition (3 Rockets + 1 Rocket Pod + 2 Force Shields); Lucky Dip (1 random weapon pickup per game, common-weighted 80/16/4); Quantum (5 Quantum Torpedoes + 2 Force Shields); Dambusters (infinite Bounce Cannons). The existing richer tiers (One at Random, Minor, One of Each, etc.) remain gated behind dev mode / campaign completion as before. Also added a Starting Armour option: None / Light (single armour layer on all stations) / Medium (double) / Heavy (triple).
...
- [x] **12.1** Add `frozen` stack counter to `Station` (integer 0–3, capped at triple frozen). Frozen is distinct from `DEAD` / `EXPLODING` — the station is still alive and visible.
...
- [x] **12.3** Fire phase: frozen stations do not fire and do not execute movement. Their `frozen` counter decrements by 1 at the end of the turn (so a single-frozen station is unfrozen next turn).
- [x] **12.4** Frozen VFX — while `frozen > 0`, cover the station with stationary ice blob particles on Layer 2. Particles spawn at the start of the frozen turn and fade individually over the turn duration, leaving the station visible underneath. Particle count scales with freeze stack (more = more dramatic).
- [x] **12.5** Frozen comet interaction — comets are destroyed on station contact and apply `frozen += 1` (capped at 3); armour absorbs the hit first (one layer consumed, no freeze). Frozen supersedes electrified: both conditions may stack but frozen takes full behavioural precedence while active.
- [x] **12.6** Team Shield and Armour still operate for frozen stations (the ship is immobilised, not defenceless). Full interaction rules documented in frozen-condition-spec.md.
- [x] **12.7** Edge cases: a frozen station that is also hit by a bullet behaves normally (takes damage / is destroyed). Frozen does not grant immunity.
...
**Status:** core implemented (entities, physics, eruptions, chains, Beam, both scenarios + wildcard, rendering). Remaining: 16.10 (AI awareness), 16.11 (design.md formula doc), bespoke sound assets, and richer eruption VFX polish (smoke trails / branching electro).

- [x] **16.1** Add `PYRO` / `CRYO` / `ELECTRO` / `BEAM` to `PlanetType` and an `isUnstable()` predicate. Passively behave as a `ROCKY` planet (gravity + obstacle + projectile-destroying impact).
- [x] **16.2** Cracked-surface rendering with pulsing under-crack glow — red (Pyro), white (Cryo), blue-cyan (Electro), yellow (Beam). Crack pattern generated once per instance from seed.
- [x] **16.3** Idle FX — mini red eruptions (Pyro), white eruptions (Cryo), crackling surface electricity (Electro), faint yellow light glints (Beam). Cosmetic only, low rate, capped.
- [x] **16.4** New `Ejecta` entity (`kind`, `owner`, `sourcePlanet`, velocity, launch delay, lifetime) and `gameState.ejecta` tracking (stagger via each ejecta's `launchDelay`).
- [x] **16.5** Eruption trigger — primary projectile (bullet/rocket) impact spawns an eruption at the contact point: 5–7 ejecta, direction = surface normal ±25°, speed = random fraction (below escape velocity), per-particle launch stagger. Cooldown prevents multi-bullet stacking.
- [x] **16.6** Ejecta flight — Pyro/Cryo ballistic and gravity-affected (per physics sub-step, like bullets); Electro travels straight, ignores gravity, short range. Global ejecta cap; consumed on planet collision / out of bounds / lifetime. Pyro ejecta **fragment asteroids** they strike (standard bullet-style shatter, credited to the instigator); cryo/electro have no terrain effect.
- [x] **16.6b** Chain reactions — an eruption payload (ejecta or Beam) that strikes an unstable planet **other than its `sourcePlanet`** triggers a fresh eruption there (subject to that planet's cooldown); striking its own source planet is silently absorbed (prevents infinite self-loop). The original `owner` propagates through the chain so all kills credit the initiating player. Bounded by cooldown + lifetime + global cap.
- [x] **16.7** Station effects with shield→armour→effect resolution and instigator attribution: Pyro destroys; Cryo `frozen += 1`; Electro `electrified += 1`. `ConditionNotifyVFX` labels for Cryo/Electro.
- [x] **16.8** Eruption VFX — Pyro fiery burst + smoke trails; Cryo icy shards + vapour; Electro branching lightning bolts (Electrostar-style arc rendering).
- [x] **16.8b** Beam type — on impact, fire a single piercing laser perpendicular to the surface, reusing the Laser-weapon path simulation + `LaserVFX` (design.md §14.2), tinted yellow. Destroys every station along the path; shatters asteroids/crystals; reflects off shields; absorbed by solid planets. On crossing another unstable planet (not its source) it triggers a fresh eruption and terminates. Kills credit the instigator (propagated through chains). Fires immediately (no `pendingLaser` delay); `ERUPTION_COOLDOWN` still applies.
- [x] **16.9** Scenario integration — add an unstable planet (random type) to the wildcard pool, and two new scenarios registered in `requirements.md` and `scenarioData.js` (bump `SCENARIO_COUNT`, add to extreme-eligible list):
...
- [x] **16.12** Mode eligibility — scenarios 39 + 40 added to `TARGET_PRACTICE_SCENARIOS` (eruptions wired into the TP firing loop). Story missions can select them via the scenario system; no bespoke Story wiring added yet.
- [x] **16.13** Eruption sound hooks (roar / ice shatter / electric zap / laser fire) at the contact point — call sites only; audio assets owned by the sound spec (§15.1).
