...
### 11.1 Mid-gesture compositing refinements (post-playtest)

A follow-up pass hardened the mid-gesture (soft) compositing path. The original scheme leaned on the cached background blit to repaint the whole canvas each frame and on the basic star disc (delta §2) to keep stars legible. Both assumptions broke in ways that only showed mid-gesture, so:

8. **The main canvas is cleared each frame.** `drawFrame` never cleared `mainCanvas`; it relied on the opaque `bgCanvas` blit to cover it. That holds at the settled camera and while zooming **in**, but `fullDeltaMatrix` scales the cached bitmap **down** when zooming out (`ratio < 1`) and shifts it past the `BG_PAD` border on fast pans, so it stops reaching the edges. The uncovered margins kept the previous frame's pixels, accumulating into a **fractured ghost trail** until the camera settled and the background re-baked to full coverage (worse and longer on the lower-frame-rate iPad). Fix: a single `fillRect('#000')` over the whole canvas at the top of `drawFrame` before compositing — one GPU-accelerated fill per frame. This makes the mid-gesture softness clean (FR-25) instead of smeared, at negligible cost.
9. **Crisp planet bodies during the gesture** (`_drawCrispPlanetBodies`). With ghosting gone, the remaining artifact was blur: the cached background's baked planet bodies go soft while it is a scaled blit. The static bodies (and their SVG overlay / polar cap / shading — passes 2–3 of `_renderBackground`) are redrawn crisply on the live layer while `_bgIsStale()`. The ~11k-gradient nebula **starfield is deliberately left soft** (its tiny fuzzy dots scale invisibly), so the per-frame cost is just a handful of vector planet draws rather than the whole expensive bake. Asteroids, gas giants, comets and moons are already drawn live; stars are covered by the disc (delta §2) plus the glow (delta §10). Gated on `_bgIsStale()`, so the settled/default frame is unchanged (FR-1).
10. **Crisp star glow during the gesture** (`PlanetRenderer.drawStarGlow` + `Renderer._drawStarGlows`). The basic star disc (delta §2) has a hard edge at the body radius; the star's coronal **glow halo** — the chromosphere ring plus five radial-gradient layers out to 3.2× the body — lives only in the baked corona (bundled with the expensive bristles + blur). While the background is a soft/ shifted blit that halo goes missing or misaligned, so on **screen-filling supergiants** the hard disc edge cut straight into black ("black sections"). Fix: redraw just the cheap glow gradients live (no offscreen, no blur, no bristles) above the soft body and below the trails/fire-rim while `_bgIsStale()`. Cost is ~6 canvas-clipped gradient fills per star per frame; the fine bristle texture stays soft and snaps crisp on settle. Decode/look unchanged at the settled view (FR-1).
