...
| 21 | White Dwarfs | 2–8 white dwarfs (no other bodies). 10% chance of extreme version (§6.4): 1–20 white dwarfs. |
...
| 37 | Giant Wormhole Network | Same layout as Star Cluster, but every star is a red network wormhole of the same size and mass — shots exit via a random other wormhole on the map. Wormholes never overlap: pairwise separation is enforced at placement, and discs that won't fit (or exceed an area budget of 55% of the screen) are dropped, trimming the body count on crowded fields. 10% chance of extreme version (§6.4). |
| 38 | Sol | Easter egg: our solar system. A mid-to-small yellow Sun (on screen or straddling an edge, always partially visible) with all nine classic bodies at increasing distances in true solar order: cratered grey Mercury, cream Venus, blue-green Earth (white polar cap, green continents, one moon), red Mars (subtle polar cap, two moons), an asteroid belt circling the Sun between Mars and Jupiter, tan-banded Jupiter, pale-gold Saturn (always ringed), pale-cyan Uranus, deep-blue Neptune, and tiny icy Pluto. Orbital spacing jitters and every body lands at a random bearing each game, so the arrangement varies while the ordering never does. |
...
- **21 (White Dwarfs):** body count 1–20 (uniform random) instead of the normal 2–8; all white dwarfs, no filler. Both versions ignore `nPlanets` and use standard random placement (no edge preference).
